...
def edit_service(req, service_id):
    if 'company' in req.session:
        ...
        if req.method == 'POST':
            ...
            service.save()
            return redirect(company_home)
        return render(req, 'company/company_service.html', {'service': service})
    return redirect(service_login)

def delete_service(req, service_id):
    if 'company' in req.session:
        service = Service.objects.get(pk=service_id)
...
def user_home(req):
    services = Service.objects.all()
    return render(req, 'user/user_home.html', {'services': services})

def book_service(req, service_id):
    if 'user' in req.session:
        ...
        return render(req, 'user/book_service.html', {'service': service})
    return redirect(service_login)
